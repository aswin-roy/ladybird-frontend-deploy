...
import { ViewState, Bill } from '../types/types';
...
export const SalesReport: React.FC<{ onNavigate: (view: ViewState) => void }> = ({ onNavigate }) => {
    ...
    const openPrintModal = (bill: Bill) => {
        setSelectedBill(bill);
                        ...
                        <div className="flex-1 overflow-y-auto bg-gray-100 p-8 flex justify-center">
                            <div id="print-preview-content" className="bg-gray-50 shadow-lg p-8 w-full max-w-md text-sm flex flex-col h-full sm:h-auto">
                                <div className="text-center mb-6">
                                    ...
                                    <p className="text-gray-500">INVOICE #{selectedBill.bill_no}</p>
                                </div>
                                <div className="mb-6 border-b border-gray-100 pb-4">
                                    <div className="flex flex-col sm:flex-row justify-between gap-4">
                                        <div>
                                            <p className="font-bold">Customer Details:</p>
                                            <p className="text-gray-900">{selectedBill.customer}</p>
                                            <p className="text-gray-500">{selectedBill.phone}</p>
                                        </div>
                                        <div className="sm:text-right">
                                            <p className="font-bold">Invoice Details:</p>
                                            <p className="text-gray-500">Date: {selectedBill.date}</p>
                                            <p className="text-gray-500">Mode: {selectedBill.paymentMode}</p>
                                        </div>
                                    </div>
                                </div>
                                <table className="w-full mb-6">
                                    <tbody>
                                        <tr>
                                            <td className="py-2">Tailoring Service</td>
                                            <td className="text-right font-bold">₹{selectedBill.amount}</td>
                                        </tr>
                                    </tbody>
                                </table>
                                <div className="border-t border-gray-200 pt-4 space-y-2 mt-auto">
                                    ...
                                    <div className="flex justify-between text-red-600 text-lg font-bold">
                                        ...
                                        <span>₹{selectedBill.amount - (selectedBill.paidAmount || 0)}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="flex items-center bg-gray-50 rounded-lg p-1 border border-gray-200">
                        <button 
                            ...
                            className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${
                                filterPeriod === 'daily' ? 'bg-gray-50 text-purple-600 shadow-sm' : 'text-gray-500 hover:text-gray-900'
                            }`}
                        >
                            Daily
                        </button>
                        <button 
                            ...
                            className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${
                                filterPeriod === 'monthly' ? 'bg-gray-50 text-purple-600 shadow-sm' : 'text-gray-500 hover:text-gray-900'
                            }`}
                        >
                            Monthly
                        </button>
