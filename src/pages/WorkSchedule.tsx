...
export const WorkSchedule: React.FC = () => {
    ...
    const stageStatusUpdate = (orderId: number | string, newStatus: string) => setStatusUpdates(prev => ({ ...prev, [String(orderId)]: newStatus }));

    const commitStatusUpdate = async (orderId: number | string) => {
        const idStr = String(orderId);
        const newStatus = statusUpdates[idStr];
        if (!newStatus) return;
                    ...
                    {Array.from({ length: daysInMonth }).map((_, i) => {
                        ...
                        return (
                            <div
                                ...
                                className={`h-32 border rounded-xl p-2 relative cursor-pointer transition-all hover:shadow-md hover:border-purple-300 group ${isToday ? 'bg-purple-50/30 border-purple-200' : 'bg-gray-50 border-gray-200'
                                    }`}
                            >
                                <span className={`absolute top-2 right-2 text-sm font-semibold w-7 h-7 flex items-center justify-center rounded-full ${isToday ? 'bg-purple-600 text-white shadow-sm' : 'text-gray-400 group-hover:text-purple-600'
                                    }`}>
                                    {day}
                                </span>
                            ...
                            {dailyOrders.length > 0 ? (
                                <div className="space-y-4">
                                    {dailyOrders.map(order => {
                                        const updateId = order._id || order.id; // Prefer MongoDB _id
                                        const stagedStatus = statusUpdates[String(updateId)];
                                                    ...
                                                    <div className="flex justify-between items-start">
                                                        ...
                                                        <div className="flex flex-col items-end">
                                                            <div className="flex items-center gap-2">
                                                                <select
                                                                    value={currentStatus}
                                                                    onChange={(e) => stageStatusUpdate(updateId, e.target.value)}
                                                                ...
                                                                {isModified && (
                                                                    <button
                                                                        onClick={() => commitStatusUpdate(updateId)}
