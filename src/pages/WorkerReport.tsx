...
export const WorkerReport: React.FC = () => {
  ...
  const handleSaveWorker = async () => {
    ...
    try {
      ...
      if (modalMode === 'add') {
        await workerService.create({ name: currentWorker.name!, role: currentWorker.role! });
      } else if (currentWorker.id) {
        await workerService.update({ id: currentWorker.id, name: currentWorker.name, role: currentWorker.role });
      }
  ...
  return (
    <div className="space-y-6 h-[calc(100vh-8rem)] flex flex-col">
          ...
          <div className="bg-gray-50 rounded-xl shadow-2xl w-full max-w-sm p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button onClick={handleSaveWorker} className="px-4 py-2 bg-purple-600 text-white rounded-lg disabled:opacity-50 flex items-center gap-2" disabled={isSubmitting}>
      ...
      <div className="overflow-auto flex-1">
        {isLoading ? (
          <div className="flex items-center justify-center h-full"><Loader2 className="animate-spin" size={24} /></div>
        ) : filteredWorkers.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-gray-400 py-12">
            <p className="text-lg font-medium">No workers found</p>
          </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {filteredWorkers.map(worker => (
                <tr key={worker.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-center">{worker.active_orders ?? 0}</td>
                  <td className="px-6 py-4 text-right">₹{worker.cutting_earnings?.toLocaleString() ?? 0}</td>
                  <td className="px-6 py-4 text-right">₹{worker.stitching_earnings?.toLocaleString() ?? 0}</td>
                  <td className="px-6 py-4 text-right">₹{worker.total_commission?.toLocaleString() ?? 0}</td>
                </tr>
              ))}
            </tbody>
          </table>
