...
export const WorkerReport: React.FC = () => {
  ...
  const openAddModal = () => { setModalMode('add'); setCurrentWorker({ name: '', role: '' }); setIsModalOpen(true); };
  const openEditModal = (worker: Worker) => { setModalMode('edit'); setCurrentWorker(worker); setIsModalOpen(true); };
              ...
              <tbody className="divide-y divide-gray-50">
                {filteredWorkers.map(worker => (
                  <tr key={worker.id}>
                    ...
                    <td className="px-6 py-4 text-right font-bold text-orange-600">₹{worker.cutting_earnings?.toLocaleString()}</td>
                    <td className="px-6 py-4 text-right font-bold text-blue-600">₹{worker.stitching_earnings?.toLocaleString()}</td>
                    <td className="px-6 py-4 text-right font-bold text-green-600">₹{worker.total_commission?.toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
