...
export const Measurements: React.FC<{ onNavigate: (view: ViewState) => void }> = ({ onNavigate }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const apiError = err as ApiError;
            alert(apiError.message || 'Failed to save measurement');
        } finally {
            setIsSubmitting(false);
        }
    };
    ...
    const uniqueCustomers = Array.from(new Set(measurements.map(m => m.customer_id)))
        .map(id => {
            const cust = measurements.find(m => m.customer_id === id);
            return cust;
        })
        .filter(Boolean)
        .filter(m => m!.customer_name?.toLowerCase().includes(searchTerm?.toLowerCase() || '')) as Measurement[];
                ...
                <>
                    <div className="flex justify-between items-center mb-4">
                        <InputField
                            ...
                            onChange={e => setSearchTerm(e.target.value)}
                        />
                        <button onClick={() => setViewMode('form')} className="bg-purple-600 text-white px-4 py-2 rounded">
                            <Plus size={18} /> New Measurement
                        </button>
                    </div>
                    {isLoading ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {uniqueCustomers.map(m => (
                                <div key={m.id} className="border p-4 rounded-lg">
                                    <h4>{m.customer_name}</h4>
                                    <p>Last update: {m.measurement_date}</p>
                                    <button
                                        onClick={() => {
                                            setCustomerName(m.customer_name);
                                            setCustomerId(m.customer_id);
            ...
            {viewMode === 'form' && (
                <div>
                    ...
                    <input
                        ...
                        onChange={e => setCustomerName(e.target.value)}
                    />
                    {/* Render your upper/lower fields here */}
